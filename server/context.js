...
const extractClientInfo = function(request) {
  ...
  // test for iOS app
  if(userAgent.toLowerCase() === 'platerateforios') {
    clientInfo.isIOS = true;
    clientInfo.isMobileApp = true;
    clientInfo.isUnknown = false;
  }
  // test for Android app
  else if(userAgent.toLowerCase() === 'platerateapp-android') {
    clientInfo.isAndroid = true;
    clientInfo.isMobileApp = true;
    clientInfo.isUnknown = false;
  }
  // test for iOS browser
  else if(_regexIOS.test(userAgent)) {
    clientInfo.isIOS = true;
    clientInfo.isBrowser = true;
    clientInfo.isUnknown = false;
  }
  // test for Android browser
  else if(_regexAndroid.test(userAgent)) {
    clientInfo.isAndroid = true;
    clientInfo.isBrowser = true;
    clientInfo.isUnknown = false;
  }
  // by default the client is a browser
  else {
    clientInfo.isBrowser = true;
    clientInfo.isUnknown = false;
  }
